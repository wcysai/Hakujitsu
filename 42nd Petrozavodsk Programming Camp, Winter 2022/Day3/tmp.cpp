...
int par[maxn], son[maxn], sz[maxn], rt, tot, n;
...
inline void insert(int u, int c){
    //tr1.upd(u, 0);
 //   tr2.upd(u, 1);
    tr2[u] += c;
    int p = u;
...
inline ui cal(int u){
    ...
    while(par[p]){
        ui dis = LCA::dis(par[p], u);
        sum += tr1[par[p]] + tr2[par[p]] * dis - (tr3[p] + tr4[p] * dis);
