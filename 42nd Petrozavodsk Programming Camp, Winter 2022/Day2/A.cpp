...
struct Tree1{
    int mn[maxn << 2],lb[maxn<<2],rb[maxn<<2];
...
struct Tree2{
    ...
    void getans(int o, int tl, int tr) {
        ...
        for (auto [op, x, y] : event[o]) 
        {
            int now=st,ans=x;
            bool des=true;
            while(T[op^1].mn[now]>T[op^1].rb[now]+x-y)
            {
                if(now==1) 
                {
                    if(T[op^1].mn[now]==100000000) ans=100000000;
                    else ans=T[op^1].mn[now]+y; des=false; break;
                }
                ans=T[op^1].rb[now]+x+1;
                now=now/2;
            } 
            if(now!=st&&des)
            {
                int minn=T[op^1].mn[now*2];
                now=now*2+1;
                while(T[op^1].lb[now]!=T[op^1].rb[now])
                {
                    if(min(minn,T[op^1].mn[now*2])>T[op^1].rb[now*2]+x-y) 
                    {
                        ans=T[op^1].rb[now*2]+x+1;
                        minn=min(minn,T[op^1].mn[now*2]);
                        now=now*2+1; 
                    }
                    else now=now*2;
                }
